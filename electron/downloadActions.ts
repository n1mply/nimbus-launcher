...
import { getMinecraftDir, readInstance, setInstanceStatus } from './instanceManager';
...
export function registerDownloadActions(mainWindow: BrowserWindow): void {
  ...
  ipcMain.handle('instance:install', async (_, instanceId: string) => {
    ...
    try {
      ...
      const javaMajor = javaService.getRecommendedJavaVersion(instance.minecraftVersion);
      await javaService.ensureJava(javaMajor, (prog) => {
        mainWindow.webContents.send('download:progress', {
          instanceId,
          statusText: 'Loading Java Runtime...',
          progress: prog.totalBytes > 0 ? Math.round((prog.downloadedBytes / prog.totalBytes) * 20) : 10,
      ...
      const { queue, totalBytesToDownload } = await integrityService.buildIntegrityQueue(
        mcDir,
        ...
        await downloadManager.downloadQueue(
          ...
          (prog) => {
            const percent = totalBytesToDownload > 0
              ? Math.round(25 + (prog.downloadedBytes / totalBytesToDownload) * 75)
              : 50;
      ...
      mainWindow.webContents.send('download:error', { instanceId, error: err.message });
      throw err;
    } finally {
