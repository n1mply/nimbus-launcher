...
async function fetchLoaderVersions(loader: Modloader, mcVersion: string): Promise<string[]> {
    switch (loader) {
        case "vanilla":
            return []
        ...
        case "fabric": {
            ...
            const data: { loader: { version: string; stable: boolean } }[] = await res.json()
...
async function loadGameVersions(): Promise<VersionManifest> {
  ...
  if (!gameVersionsPromise) {
    gameVersionsPromise = fetch('https://piston-meta.mojang.com/mc/game/version_manifest_v2.json')
      .then((res) => res.json())
      .then((data) => {
        gameVersionsCache = data
        return data
      })
      .finally(() => { gameVersionsPromise = null })
  }
...
export function registerVersionsHandlers() {
  ...
  ipcMain.handle('versions:getLoaderVersions', (_event, loader: string, mcVersion: string) =>
