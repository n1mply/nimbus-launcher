...
export default function AddInstanceModal({
  ...
  const [loaderVersionMode, setLoaderVersionMode] = useState<
    "latest" | "other"
  >("latest");
  ...
  const handleModloaderSelect = (loader: Instance["modloader"]) => {
    ...
    if (minecraftVersion) {
      const isStillValid = filteredGameVersions.some(
        (v) => v.value === minecraftVersion,
      );
      if (!isStillValid) setMinecraftVersion("");
    }
  };
  ...
  const filteredGameVersions = useMemo(() => {
    const minVersion = getMinMinecraftVersion(modloader);
    if (!minVersion) return parsedVersions;

    const minIndex = parsedVersions.findIndex((v) => v.value === minVersion);

    return minIndex !== -1
      ? parsedVersions.slice(0, minIndex + 1)
      : parsedVersions;
  }, [parsedVersions, modloader]);
  ...
  useEffect(() => {
    const handleParseVersions = () => {
      ...
      setParsedVersions(parsed);
    };
    handleParseVersions();
  }, [versions]);

  useEffect(() => {
    if (modloader !== "vanilla" && minecraftVersion.length > 0) {
      fetchLoaderVersions(modloader, minecraftVersion);
    } else {
      setLoaderVersions([]);
    }
  }, [modloader, minecraftVersion]);

  const parsedLoaderVersions = useMemo(() => {
    return loaderVersions.map((version) => ({
      label: version,
      value: version,
    }));
  }, [loaderVersions]);
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            placeholder={
              minecraftVersion
                ? `${modloaderLabel} ${minecraftVersion}`
                : "Instance name"
            }
